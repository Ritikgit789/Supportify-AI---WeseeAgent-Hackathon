...
def chunk_with_overlap(kb_data: Union[str, List[Dict]], chunk_size=700, chunk_overlap=100):
    """
    Accepts either a path to a JSON file or the loaded JSON data.
    """
    if isinstance(kb_data, str):
        # If a path is given, load the JSON file
        with open(kb_data, 'r', encoding='utf-8') as f:
            raw_data = json.load(f)
    else:
        # If data is already loaded (from ZIP), use it directly
        raw_data = kb_data
...
def generate_response(query: str, context: str, category: str) -> str:
    prompt = ChatPromptTemplate.from_template(
        ...
        "Query: {query}\n\n"
        "If asked about service hours, please say - Our service hours are from 9 AM to 6 PM. After hours, please email support@supportify.com.\n"
...
def answer_out_of_kb(query: str, category: str) -> str:
    prompt = ChatPromptTemplate.from_template(
        "You are a helpful {category} support agent. "
        "The knowledge base does not contain relevant information for this query. "
        "Please answer using your general knowledge and best judgment.\n\n"
        "Query: {query}\n\n"
        "If the user asks about service hours, say: 'Our service hours are from 9 AM to 6 PM. After hours, please email support@supportify.com.'\n"
        "If the user asks about general trends, SaaS, or mobile apps, give a concise, helpful answer.\n"
        "If you don't know, politely say so."
        "Don't say directly I don't know"
    )
    return (prompt | llm).invoke({"query": query, "category": category}).content.strip()
...
def run_customer_support(query: str, kb_chunks: List[Dict]) -> Dict[str, str]:
    ...
    if sentiment == "Negative" and is_critical:
        ...
        if relevant_chunks:
            ...
            response = answer_out_of_kb(query, category)
    return {
        "category": category,
        "sentiment": sentiment,
        "response": response
...
if __name__ == "__main__":
    print("Loading knowledge base from ZIP...")
    kb_data = load_kb_from_zip("knowledge_base.zip", "knowledge_base.json")
    kb_chunks = chunk_with_overlap(kb_data)
